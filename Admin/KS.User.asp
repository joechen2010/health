...
Class Admin_User
       ...
       Sub Kesion()
			...
			Response.Write "<div id='mtl'>快速查找用户：</div><li><a href=""KS.User.asp?Action=Search"">搜索用户</a></li>&nbsp;|&nbsp;<a href=""?UserSearch=12"">所有用户</a>&nbsp;|&nbsp;<a href=""?UserSearch=1"">被锁住的用户</a>&nbsp;|&nbsp;<a href=""?UserSearch=2"">所有管理员</a>&nbsp;|&nbsp;<a href=""?UserSearch=3"">待审批会员</a>&nbsp;|&nbsp;<a href=""?UserSearch=4"">待邮件激活</a>&nbsp;|&nbsp;<a href=""?UserSearch=5"">24小时内登录</a>&nbsp;|&nbsp;<a href=""?UserSearch=6"">24小时内注册</a>"
			Response.Write	" </ul>"
		...
		%>
		  <table width="100%" style="border-top:1px #CCCCCC solid" border="0" align="center" cellspacing="0" cellpadding="0">
				...
				if SQL(3,i)=1 then
					if SQL(4,i)<=0 then
						Response.Write "<font color=#ff6600>" & SQL(4,i) & "</font> 点"
					else
						if SQL(4,i)<=10 then
							Response.Write "<font color=blue>" & SQL(4,i) & "</font> 点"
						else
				...
				If KS.C("SuperTF")<>1 and SQL(2,i)=4 and  SQL(1,i)<>KS.C("AdminName") then
					...
					if SQL(2,i)<>1 then  '管理员判断
						...
						Response.Write "<a href='KS.User.asp?Action=Del&UserID=" & SQL(0,i) & "' onClick='return confirm(""确定要删除此用户吗？"");'>删</a>&nbsp;"
						 If SQL(3,i)=1 Then
						   Response.Write "<a href='KS.User.asp?Action=AddMoney&UserID=" & SQL(0,i) & "'>续点数</a>"
						 ElseIf SQL(3,I)=2 Then
			...
			<TR class="tdbg"> 
			  ...
			  <TD> <Input class="textbox" Name="UserName" id="UserName" type=text size=20> <font color="red">*</font><input type="button" name="Submit22" value="检测帐号"  onClick="checkaccount($('#UserName').val())" class="button"></TD>
				...
				<TR class="tdbg"> 
				  ...
				  <TD height="25"><INPUT class="textbox" type="password" name="Password" value="" size="30" maxLength="12"><font color="red">*</font><br><font color="#FF6600">用户登录时的密码</font></TD>
					<TD width="80" height="25" align="right" class="clefttitle"><strong>重复密码：</strong></TD>
		...
		<FORM name="myform" action="KS.User.asp" method="post">
			...
			<TR class="tdbg"> 
			  ...
			  <TD> <Input class="textbox" Name="UserName" type=text size=30 Value="<%=rsUser("UserName")%>" readonly> <font color="red">*</font></TD>
				...
				<TR class="tdbg"> 
					<TD width="80" height="25" align="right" class="clefttitle"><strong>用户密码：</strong></TD>
				  <TD height="25"><INPUT class="textbox" type="password" name="Password" value="" size="30" maxLength="12"><br><font color="#FF6600">如果不想修改，请留空</font></TD>
					<TD width="80" height="25" align="right" class="clefttitle"><strong>重复密码：</strong></TD>
					<TD height="25"><INPUT class="textbox" type="password" name="PwdConfirm" value="" size="30" maxLength="12"><br><font color="#FF6600">如果不想修改，请留空</font></TD>
				</TR>
			<TR  class="tdbg"> 
				<TD width="80" height="25" align="right" class="clefttitle"><strong>密码问题：</strong></TD>
			...
			<TR class="tdbg"> 
			  ...
			  <TD height="25" align="right"><strong>用户状态：</strong></TD>
...
无限期</TD>
			    <TD align="right"><strong>头像地址：</strong></TD>
			...
			<TR class="tdbg"> 
			  ...
			  <TD> <Input class="textbox" Name="Money" type=text size=8 Value="<%=rsUser("Money")%>">元</TD>
			</TR>
		...
		<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
			...
			<TR class="tdbg"> 
				...
				<strong>用户名称：</strong><%=rsUser("UserName")%>&nbsp;&nbsp;<strong>注册时间：</strong><%=rsUser("RegDate")%>&nbsp;&nbsp;<strong>推荐人：</strong><%=rsUser("AllianceUser")%>&nbsp;&nbsp;<strong>可用资金：</strong><%=rsUser("money")%>元&nbsp;&nbsp;<strong>可用点券：</strong><%=rsUser("point")%>点&nbsp;&nbsp;<strong>积分：</strong><%=rsUser("score")%>分
				</TD>
			</TR>
	  ...
	  End Sub
		...
		<FORM name="myform" action="KS.User.asp?ComeUrl=<%=Server.URLEncode(Request.ServerVariables("HTTP_REFERER"))%>" method="post">
			...
			<%if rsUser("ChargeType")=1 then%>
			<TR  class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>目前的用户点数：</strong></TD>
			  <TD><%=rsUser("Point")%> 点</TD>
			</TR>
			<TR  class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>追加点数：</strong></TD>
...
若目前用户已经过了有效期，则有效期从续费之日起重新计数。</td>
			...
			<TR class='tdbg'>
			  ...
			  <TD> <input name="Reason" type="text" id="Reason" value="<%If rsUser("ChargeType")=1 Then Response.Write "续点券操作" Else Response.Write "续有效天数操作"%>" size="55"></TD>
			</TR>
			<TR class='tdbg'> 
			  <TD height="40" colspan="2" align="center"><input name="Action" type="hidden" id="Action" value="SaveAddMoney"> 
			  <input name=Submit   type=submit id="Submit" value="&nbsp;保存续费结果&nbsp;" > <input name="UserID" type="hidden" id="UserID" value="<%=rsUser("UserID")%>"></TD>
			</TR>
		</form>
	    </TABLE>
		<%
		...
		sub SaveAddMoney()
			...
			If Money>0 Then
			 if ChargeType=2 Then
			  Call KS.MoneyInOrOut(rsUser("UserName"),rsUser("RealName"),Money,4,2,now,0,KS.C("AdminName"),"用于兑换有效天数",0,0)
			 else
			  Call KS.MoneyInOrOut(rsUser("UserName"),rsUser("RealName"),Money,4,2,now,0,KS.C("AdminName"),"用于兑换点券",0,0)
			 end if
			end if
		...
		<FORM name="myform" action="KS.User.asp?ComeUrl=<%=Server.URLEncode(Request.ServerVariables("HTTP_REFERER"))%>" method="post">
			...
			<TR  class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>汇款日期：</strong></TD>
			  <TD><input name="PayTime" type="text" id="PayTime" value="<%=formatdatetime(now,2)%>" size="15" class="textbox"></TD>
			</TR>
			<TR  class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>续费金额：</strong></TD>
		...
		sub SaveAddZJ()
            ...
            If Not IsDate(PayTime) Then
				FoundErr=True
				ErrMsg=ErrMsg & "<br><li>汇款日期格式有误</li>"
			end if
			if KS.ChkClng(Money)=0 then
				FoundErr=True
				ErrMsg=ErrMsg & "<br><li>请输入要续费金额</li>"
			end if
		    if Remark="" Then
			  FoundErr=True
