...
Class Admin_Field
		...
		.Write "        </tr>"
			...
			End If
		 .Write " <tr>"
		 .Write "   <td colspan='3'>&nbsp;&nbsp;<input type='button' value='表单预览' onclick=""SelectObjItem1(this,'自定义表单 >> <font color=red>表单预览</font>','gosave','KS.Form.asp?ItemID=" & ItemID & "&action=view');"" class='button'>&nbsp;<input type='submit' class='button' value='批量保存排序'> <font color=blue>越小排在越前面</font></td></form>"
		 .Write "   <td height='35' colspan='4' align='right'>"
		 Call KSCLS.ShowPage(totalPut, MaxPerPage, "KS.FormField.asp", True, "个",CurrentPage, "ItemID=" & ItemID)
		.Write "    </td>"
		.Write " </tr>"
		.Write "</table>"
		.Write "</div>"
	  ...
	  End If
		...
		.Write "      <td align='right' ' class='clefttitle'><strong>列表选项：</strong><br><font color=blue>每一行为一个列表选项</font></td>"
		...
		.Write "      <td nowrap>&nbsp;&nbsp;宽度<input name='Width' type='text' site='10' class='textbox' style='width:40px' value='" & Width & "'>px &nbsp;高度<input name='Height' type='text' site='10' class='textbox' style='width:40px' value='" & Height & "'>px</font>"
		.Write "       </td>"
		.Write "    </tr>"				
		.Write "    <tr class=""tdbg"" onMouseOver=""this.className='tdbgmouseover'"" onMouseOut=""this.className='tdbg'"">"
		...
		End Sub
		 ...
		 Sub FieldOrder()
			  ...
			  Next
			  Response.Write "<script>alert('批量保存字段排序成功！');location.href='KS.FormField.asp?ItemID=" & ItemID&"&Page=" & Page & "';</script>"
		 End Sub
End Class
%> 
