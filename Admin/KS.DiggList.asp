...
%>	   		
     <SCRIPT language=javascript>
		...
		function show(t,m,d)
		{
		PopupCenterIframe('查看详情[<font color=red>'+t+'</font>]记录','KS.DiggList.asp?action=list&infoid='+d+'&ChannelID='+m,750,440,'auto')
		}
		function ShowCode(){
		PopupCenterIframe('查看Digg调用代码','KS.DiggList.asp?action=ShowCode',750,440,'no')
		}
	   ...
	   <%
		...
		.echo "  </tr>"
	     ...
	     .echo ("</form><td align='right'>")
	      Call KSCLS.ShowPage(totalPut, MaxPerPage, "KS.DiggList.asp", True, "条", CurrentPage,KS.QueryParam("page"))
		...
		Sub showContent()
		  ...
		  .echo " <td class='splittd' height='22'><img src='Images/folder/TheSmallWordNews1.gif' align='absmiddle'><span style='cursor:default;'>"
		   .echo  RS("Title") & "</td>"
	...
	.mark {overflow:hidden;padding:15px 0 20px 111px; clear:both;}
	#mark0, #mark1 {background:url({$GetSiteUrl}images/mark.gif) no-repeat -189px 0;border:0;cursor:pointer;float:left;height:48px;margin:0;overflow:hidden;padding:0;position:relative;width:189px;}
	#mark1 {background-position:-378px 0;margin-left:10px;}
	#barnum1, #barnum2 {color:#333333;font-family:arial;font-size:10px;font-weight:400;left:70px;line-height:12px;position:absolute;top:30px;}
	.bar {background-color:#FFFFFF;border:1px solid #40A300;height:5px;left:9px;overflow:hidden;position:absolute;text-align:left;top:32px;width:55px;}
...
<div style="float:left;BACKGROUND: url(../images/ding_bg.gif) no-repeat; WIDTH: 58px; HEIGHT: 62px">
...
<div style="float:right;BACKGROUND: url(../images/ding_bg.gif) no-repeat; WIDTH: 58px; HEIGHT: 62px">
...
<div style="width:153px">									
<div style="float:left;BACKGROUND: url({$GetSiteUrl}images/ding_bg.gif) no-repeat; WIDTH: 58px; HEIGHT: 62px">
...
<div style="float:right;BACKGROUND: url({$GetSiteUrl}images/ding_bg.gif) no-repeat; WIDTH: 58px; HEIGHT: 62px">
...
<script language="JavaScript" type="text/javascript">show_digg({$ChannelID},{$InfoID},'{$GetSiteUrl}');</script>
	   ...
	   <%
		 ...
		 Sub DelAllRecord()
          ...
          KS.echo "<script>$(top.frames['FrameTop'].document).find('#ajaxmsg').toggle();alert('恭喜,删除指定日期digg记录成功!');</script>"
		 End Sub
		 ...
		 Sub DiggDetail()
		  With KS
			...
			.echo "<form name=""myform"" id=""myform"" method=""Post"" action=""KS.DiggList.asp?action=DiggDel&ChannelID=" & ChannelID & """>"
					 ...
					 Else
								...
								If IsObject(XML) Then 
								  For Each Node In XML.DocumentElement.SelectNodes("row")
									 ...
									 if Node.SelectSingleNode("@username").text = "1" then
									  .echo "顶"
									 else
									  .echo "踩"
									 end if
			 ...
			 .echo ("</form><td align='right'>")
				Call KSCLS.ShowPage(totalPut, MaxPerPage, "KS.DiggList.asp", True, KS.C_S(ChannelID,4), CurrentPage,KS.QueryParam("page"))
			 .echo ("</td></tr></form></table>")
		End With
