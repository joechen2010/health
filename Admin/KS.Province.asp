...
<script src="../KS_Inc/Kesion.box.js"></script>
			<script language="javascript">
				...
				function AreaAdd()
				{
						 PopupCenterIframe('新增地区','?Action=add&parentid=<%=ks.s("parentid")%>',630,200,'no')
				}
				function EditArea(id)
				{
					 PopupCenterIframe('编辑地区','KS.Province.asp?Action=add&ID='+id,630,200,'no')
				}
				function DelArea(id)
				{
...
%>
 ...
 <td colspan=5>
	  ...
	  <%
	  Call KSCLS.ShowPage(RCount, Psize, "KS.Province.asp", True, "个", Page, KS.QueryParam("page"))
	  %> </td>
	  </tr>
</table>
</body>
...
Sub Add_Submit_Save()
 ...
 Call CloseConn()
 If Id=0 Then
 Response.write "<script>if (confirm('添加成功,继续添加吗?')){location.href='KS.Province.asp?action=add&parentid=" & parentid & "';}else{top.frames[""MainFrame""].location.reload();}</script>"
 ELse
 Response.write "<script>alert('修改成功！');top.frames[""MainFrame""].location.reload();</script>"
 end if
 Response.End()
End Sub
