...
Class Admin_Field
		...
		.Write "        </tr>"
		 ...
		 Call KSCLS.ShowPage(totalPut, MaxPerPage, "KS.Field.asp", True, "个",CurrentPage, "ChannelID=" & ChannelID)
		.Write "    </td>"
		.Write " </tr>"
		.Write "</table>"
		.Write "</div>"
	  ...
	  End If
		...
		.Write "      <td nowrap>&nbsp;&nbsp;<input name='EditorType' type='text' id='EditorType' class='textbox' value='" & EditorType & "' size='10'>&nbsp;<select onchange=""$('EditorType').value=this.value"" name='selecteditor'><option value='Default'>Default</option><option value='NewsTool'>NewsTool</option><option value='Simple'>Simple</option><option value='Basic'>Basic</option></select><span style='color:green'>您可以打开/KS_Editor/fckeditor/fckconfig.js自定义编辑器类型</span>"
		.Write "       </td>"
		.Write "    </tr>"
		.Write " </tbody>"
		...
		If ChannelID<>101 Then
		 .Write "<div>&nbsp;&nbsp;<font color=green>为便于会员获取默认值，可绑定表KS_User或KS_Enterprise的字段值<br>&nbsp;&nbsp;格式：表名|字段名 如：<font color=red>KS_User|RealName</font></font><br/>&nbsp;&nbsp;<font color=blue>也可以将默认值设置为now或date取得当前时间</font></div>"
		End If
		.Write "       </td>"
		.Write "    </tr>"
		...
		.Write "      <td nowrap>&nbsp;&nbsp;宽度<input name='Width' type='text' site='10' class='textbox' style='width:40px' id='Width' value='" & Width & "'>px <font color=red>例如：200px</font><br><span style='display:none' id='heightarea'>&nbsp;&nbsp;高度<input name='Height' type='text' site='10' class='textbox' style='width:40px' id='Height' value='" & Height & "'>px <font color=red>例如：100px</font></span>"
		.Write "       </td>"
		.Write "    </tr>"		
		.Write "    <tr class=""tdbg"" onMouseOver=""this.className='tdbgmouseover'"" onMouseOut=""this.className='tdbg'"">"
