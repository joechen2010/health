...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
	  ...
	  do while not rssort.eof
	  %>
        <td height="25"><img src="images/12.gif" width="6" height="10">&nbsp;<%=rssort("ClassName")%><span class="style1">〖<a href="?action=edit&id=<%=rssort("id")%>"><span class="style1">修改</span></a>│<a href="?pid=<%=ks.g("id")%>&action=del&id=<%=rssort("id")%>&flag=sub" onclick="return(confirm('你确定要删除该类吗？'))"><span class="style1">删除</span></a>〗</span></td>
		<%
		if i mod 3<>0 then
		end if
		if i mod 3=0 then%>
