...
Class Admin_Origin
		...
		function OriginAdd()
		{
		  PopupCenterIframe('新增来源','KS.Origin.asp?Action=Add',630,410,'no')
		}
		function EditOrigin(id)
		{ 
		PopupCenterIframe('编辑来源',"KS.Origin.asp?action=Edit&ID="+id,630,410,'no')
		}
		...
		<%
		 ...
		 .echo "        </tr>"
			...
			End If
	         ...
	         Call KSCls.ShowPage(totalPut, MaxPerPage, "KS.Origin.asp", True, "位", CurrentPage, "")
	         .echo ("</td></tr></table>")
		End With
		End Sub
		Sub showContent()
   ...
   End If
		...
		End Sub
		 ...
		 If RS.EOF And RS.BOF Then
		  ...
		  RS("AddDate") = Now()
		  RS.Update
		  KS.Echo ("<Script> if (confirm('来源增加成功,继续添加吗?')) { location.href='KS.Origin.asp?Action=Add';} else{top.frames[""MainFrame""].location.reload();}</script>")
		 Else
		 ...
		 Set RS = Server.CreateObject("ADODB.RECORDSET")
		  ...
		  RS.Update
		  RS.Close
		  KS.Echo ("<Script> alert('来源修改成功!');top.frames[""MainFrame""].location.reload();</script>")
		 End With
		 End Sub
