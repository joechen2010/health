...
Class Admin_KeyWord
		...
		Sub KeyWordList()			
			...
			<script language="javascript">
				...
				function KeyWordAdd()
				{
					PopupCenterIframe('新增关键字Tags','KS.KeyWord.asp?Action=Add',530,110,'no')
				}
				function EditKeyWord(id)
				{	
				   PopupCenterIframe('新增关键字Tags','KS.KeyWord.asp?Page='+Page+'&Action=Edit&ID='+id,530,110,'no')
		 ...
		 .echo "</div>"
			...
			Sub DoSave()
				...
				If KeyWordText = "" Then
				 ...
				 RS.Close:Set RS=Nothing
				 If ID=0 Then
				  KS.Echo ("<Script> if (confirm('关键字增加成功,继续添加吗?')) { location.href='?Action=Add';} else{top.frames[""MainFrame""].location.reload();}</script>")
				 Else
				  KS.Echo ("<Script> alert('关键字修改成功!');top.frames[""MainFrame""].location.reload();</script>")
				 End If
			End Sub
			...
			Sub DelAllRecord()
			  ...
			  End If
			  Conn.Execute(SQL)
               KS.echo "<script>$(top.frames['FrameTop'].document).find('#ajaxmsg').toggle();alert('恭喜,删除指定条件的记录成功!');</script>"
			End Sub
End Class
%>
