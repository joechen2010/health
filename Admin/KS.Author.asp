...
Class Admin_Author
	...
	%>
	   <script language="javascript">
		...
		function AuthorAdd()
		{
		 PopupCenterIframe('新增作者/开发商','KS.Author.asp?ChannelID='+ChannelID+'&Action=Add',630,410,'no')
		}
		function EditAuthor(id)
		{
		  PopupCenterIframe('编辑作者','KS.Author.asp?ChannelID='+ChannelID+'&Action=Edit&ID='+id,630,410,'no')
		}
	   ...
	   With KS
		...
		.echo "  </tr>"
		 Set RS = Server.CreateObject("ADODB.RecordSet")
	        ...
	        .echo ("</form><td align='right'>")
	         Call KSCls.ShowPage(totalPut, MaxPerPage, "KS.Author.asp", True, "位", CurrentPage, "ChannelID=" & ChannelID)
	        .echo ("</td></tr></table>")
		End With
		End Sub
		Sub showContent()
        ...
        End With
		 ...
		 If RS.EOF And RS.BOF Then
		  ...
		  RS.Update
		  Set conn = Nothing
		  KS.Echo ("<Script> if (confirm('作者增加成功,继续添加吗?')) { location.href='KS.Author.asp?ChannelID="& ChannelID& "&Action=Add';} else{top.frames[""MainFrame""].location.reload();}</script>")
		 Else
		 ...
		 Dim OriginSQL:OriginSql = "Select * From [KS_Origin] Where ID='" & ID & "'"
		  ...
		  Set RS=Nothing
		   ...
		   KS.Echo ("<Script> alert('作者修改成功!');top.frames[""MainFrame""].location.reload();</script>")
		   Set conn = Nothing
		  End With
		 End Sub
