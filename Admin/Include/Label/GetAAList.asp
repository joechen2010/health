...
Class GetAAList
		...
		Dim ClassID, OpenType, SolutionTF, ShowClass,ShowUserName,Num, ZWLen, TitleLen, InfoSort, ColNumber, Province, NavType, Navi, DateRule, DateAlign, TitleCss, City,ShowStyle, PrintType,AjaxOut,LabelStyle
		...
		Else
		  ...
		  End If
			...
			LabelContent       = Replace(Replace(LabelContent, "{Tag:GetAAList", ""),"}" & LabelStyle & "{/Tag}", "")
			Dim XMLDoc,Node
			...
			SolutionTF=Node.getAttribute("SolutionTF")
		...
		If KS.IsNul(SolutionTF) Then SolutionTF=2
       ...
       function InsertLabel(label)
		...
		function CheckForm()
		{
			...
			var SolutionTF=$("input[name=SolutionTF][checked=true]").val();
            ...
            var tagVal='{Tag:GetAAList labelid="0" classid="'+ClassID+'" showstyle="'+ShowStyle+'" SolutionTF="'+SolutionTF+'" opentype="'+OpenType+'" num="'+Num+'" titlelen="'+TitleLen+'" infosort="'+InfoSort+'" showclass="'+ShowClass+'" showusername="'+ShowUserName+'" showuserface="'+ShowUserFace+'" splitpic="'+SplitPic+'" navtype="'+NavType+'" nav="'+Nav+'" titlecss="'+TitleCss+'" daterule="'+DateRule+'" printtype="'+PrintType+'" ajaxout="'+AjaxOut+'"}'+$("#LabelStyle").val()+'{/Tag}';
		...
		.echo " <input type=""hidden"" name=""FileUrl"" value=""GetAAList.asp"">"
		...
		.echo "            <tr class=tdbg>"
		.echo "              <td colspan='2' id='ShowFieldArea' class='field'><li onclick=""InsertLabel('{@autoid}')"">行 号</li><li onclick=""InsertLabel('{@aqurl}')"">问题URL</li> <li onclick=""InsertLabel('{@title}')"">标题</li><li onclick=""InsertLabel('{@username}')"">回答者</li><li onclick=""InsertLabel('{@userface}')"">回答头像</li><li onclick=""InsertLabel('{@aqclassname}')"">分类名称</li><li onclick=""InsertLabel('{@aqclassid}')"">分类id</li><li onclick=""InsertLabel('{@aqclassurl}')"">分类Url</li><li onclick=""InsertLabel('{@adddate}')"">回答时间</li></td>"
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
	...
	Set Rs = Nothing
		...
		.echo "              </td>"
		.echo "              <td width=""50%"" height=""24"">条件"
		.echo "                <input name=""SolutionTF"" type=""radio"" value=""2"""
		If SolutionTF = "2" Then .echo (" Checked")
		.echo ">不限"				  
		.echo "                <input name=""SolutionTF"" type=""radio"" value=""1"""
		If SolutionTF = "1" Then .echo (" Checked")
		.echo ">仅显示已解决"				  
		.echo "                <input name=""SolutionTF"" type=""radio"" value=""0"""
		If SolutionTF = "0" Then .echo (" Checked")
		.echo ">仅显示未解决"				  
		...
		.echo "                <select style=""width:70%;"" class='textbox' name=""InfoSort"">"
					If InfoSort = "a.AnswerID Desc" Then
					 .echo ("<option value='a.AnswerID Desc' selected>回答ID降序</option>")
					Else
					 .echo ("<option value='a.AnswerID Desc'>回答ID降序</option>")
					End If
					If InfoSort = "a.AnswerID asc" Then
					 .echo ("<option value='a.AnswerID asc' selected>回答ID升序</option>")
					Else
					 .echo ("<option value='a.AnswerID asc'>回答ID升序</option>")
					End If
		 ...
		 .echo "             <td colspan=2 height=""30"">附加显示 "
				   ...
				   End If
					 ...
					 If  cbool(ShowUserName) = True Then
					  .echo ("<input type=""checkbox"" value=""true"" id=""ShowUserName"" name=""ShowUserName"" checked>显示回答者")
					 Else
					  .echo ("<input type=""checkbox"" value=""true"" id=""ShowUserName"" name=""ShowUserName"">显示回答者")
					 End If
                      .echo "&nbsp;&nbsp;&nbsp;&nbsp;"
					 If  cbool(ShowUserFace) = True Then
					  .echo ("<input type=""checkbox"" value=""true"" id=""ShowUserFace"" name=""ShowUserFace"" checked>显示回答者头像")
					 Else
					  .echo ("<input type=""checkbox"" value=""true"" id=""ShowUserFace"" name=""ShowUserFace"">显示回答者头像")
					 End If
