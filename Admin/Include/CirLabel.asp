...
%>
       <table border='0' cellspacing='1' cellpadding='1' width='98%' align='center' class='ctable'>
		  ...
		  <tr class='tdbg'>
		   ...
		   <td><%=KS.ReturnLabelFolderTree(FolderID, 6)%><font color=""#FF0000"">请选择标签归属目录，以便日后管理标签</font></td>
		  </tr>
...
<%
		...
		rsc.close
		set rsc=nothing
		.echo " </select>日期样式 <select style='width:150px'  class=""textbox"" name=""DateRule"" id=""DateRule"">"
		...
		Sub DoSave()
					...
					Else
						...
						ks.echo ("<script>if (confirm('成功提示:\n\n添加标签成功,继续添加标签吗?')){location.href='CirLabel.asp?Action=AddNew&LabelType=6&FolderID=" & FolderID & "';}else{$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 循环标签&ButtonSymbol=FreeLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=6&FolderID=" & FolderID & "';}</script>")
						Else
						 	 '遍历所有标签内容，找出所有标签的图片
						...
						ks.echo ("<script>alert('成功提示:\n\n标签修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 循环标签&ButtonSymbol=FreeLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=6&FolderID=" & FolderID & "';</script>")
						End If
					End If
			End Sub
End Class
