...
<BODY bgcolor="#3366CC" onLoad="fload();SetTimeInput();">
<SCRIPT>
...
function fSetDate(iYear, iMonth, iDay,flag){
  ...
  if ((iYear == 0) && (iMonth == 0) && (iDay == 0)){
    ...
    iMonth = iMonth + 100 + "";
  	...
  	gdCtrl.value = iYear+"-"+iMonth+"-"+iDay+' '+TimeStr;
  }
  else{
  	iMonth = iMonth + 100 + "";
  	iMonth = iMonth.substring(1);
...
function SetTimeInput()
{
	...
	document.all.nowbutton.value=TimeStr;
}
function HiddenDiv()
{
	var i;
...
write("<B ID=\"CAL_B_BLANK\" style='color:"+gcBG+"; visibility:visible; cursor:pointer; font-size:12px' onclick='fSetDate(giYear,giMonth,giDay,9999)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gcBG'><input type='button' value='当前时间' name='nowbutton'></B>");
write("</td><td algin='center'>");
write("<B style='color:"+gcBG+";cursor:pointer; font-size:12px' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gcBG'>选择: "+giYear+"/"+giMonth+"/"+giDay+"</B>");
write("</td></tr></table>");
write("</TD></TR>");
write("</TABLE></Div>");
}
