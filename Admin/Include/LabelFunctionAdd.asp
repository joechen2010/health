...
Class DIYFunction
		...
		<br><br>
	     <table border='0' cellspacing='1' cellpadding='1' width='95%' align='center' class='ctable'>
		  ...
		  <tr class='tdbg'>
		   ...
		   <td><%=KS.ReturnLabelFolderTree(FolderID, 5)%><font color=""#FF0000"">请选择标签归属目录，以便日后管理标签</font></td>
		  </tr>
		  <tr class='tdbg'>
		  ...
		  <tr class='tdbg' id='lbt'>
		   ...
		   <td>
			...
			<table border='0' id='pagearea' <%if sqltype=0 then .echo  " style=display:none"%>>
			 <tr><td>分页项目单位：<input type="text" value="<%=itemname%>" class="textbox" name="ItemName" size="6"> 如：篇、组、个、部等</td><td width='250'>&nbsp;&nbsp;&nbsp;<%=KS.ReturnPageStyle(PageStyle)%></td>
			 </tr>
			 </table>
			</td>
	...
	function AddParamToSql2(input){
	  ...
	  function CheckForm()
		...
		function ShowIframe()
		{   PopupImgDir="../";
			PopupCenterIframe("查看栏目<=>ID对照表","?action=ShowClassID",600,350,"auto")
		}
		</script>
	  <%
	   ...
	   With KS
		...
		<table style="margin-top:5px" width="100%" border=0 cellpadding='2' cellspacing='1' class='border'>
			...
			<tr class="tdbg" <%If dbname1<>"" and KS.G("dbname2")<>"" then .echo "" else .echo " style='display:none'"%>>
			  <td height="28" align=middle><strong>约束字段：</strong></td>
		  ...
		  <%
			...
			<tr class="tdbg">
			  <td align=middle width=100><strong>选择字段：</strong><br><br><font color=#ff0000>请选择需要调用的字段名称,按Ctrl或Shift键多选</font></td>
			  <td width=100>
		...
		<Select class="textbox" style="WIDTH: 250px; HEIGHT: 210px" onChange='addfield()' multiple size=2 name="field2">
		  ...
		  if KS.G("dbname2")="" then .echo "<Option value=0>请先选择一个表</Option>"
		    ...
		    <tr class="tdbg">
		      <td align='center'><strong>参数说明：</strong></td>
	      ...
	      <tr class="tdbg">
            <td width='100' align='center'><strong>查询语句：</strong></td>
			...
			<font color=red>特别提示：</font>
			 <li>1.支持使用{ReqNum(字符串)}或{ReqStr(字符串)}来取得Url的参数值</font><br><font color=blue>如：http://www.kesion.com/index.asp?ClassID=100,那么sql语句：select top 10 foldername from ks_class where classid={ReqNum(ClassID)} 将自动转换为select top 10 foldername from ks_class where classid=100</font>
			 </li>
			 <li>2.循环体支持使用IF条件语句,详看<a href="http://www.kesion.com/tech/v5/73.html" target="_blank">http://www.kesion.com/tech/v5/73.html</a></li>
			 <li>3.当个人/企业空间要使用sql标签时,可以用<font color=red>"{$GetUserName}"</font>取得当前空间的用户名
			 <br><font color=blue>如:select top 10 id,title from ks_article where inputer='{$GetUserName}' order by id desc</font></li>
			</td>
	   </tr>
	...
	Function GetCommonField(ChannelName,BasicType,byref CommonFieldArr,itemname)
	  select case BasicType
		 ...
		 Case 5
		  ...
		  GetCommonField=GetCommonField &"<option value=""Price"">·当前价格</option>"
		  GetCommonField=GetCommonField &"<option value=""Price_Member"">·会员价格</option>"
		  GetCommonField=GetCommonField &"<option value=""Price_Market"">·市场价格</option>"
		  GetCommonField=GetCommonField &"<option value=""Price_Original"">·原始零售价</option>"
		  GetCommonField=GetCommonField &"<option value=""Discount"">·折扣</option>"
	...
	End Function
	 ...
	 Sub Step2()
		...
		{ 
		  ...
		  PopupImgDir="../";
		  PopupCenterIframe('插入字段标签',link,350,230,'no');
		}
       ...
       <script src="../../ks_inc/kesion.box.js"></script>
		...
		End If
		 ...
		 .echo "	<td><strong>简要描述</strong></td><td><font color=red>1、SQL标签定义规则</font><br>循环体格式：[loop=n]循环标签的内容[/loop]<br>其中n表示循环次数，且n满足n>=0。loop为循环关键字，此循环体可以重复使用,但是不能嵌套。<font color=red><br>2、SQL标签字段规则</font><br>字段格式：{$Field(FieldName,OutType,Param,...)}<br>FieldName&nbsp;&nbsp;--数据库表的字段名称<br>OutType&nbsp;&nbsp;&nbsp;&nbsp;--输出类型 支持：文本(Text)、日期(Date)、数据(Num)、对象URL(GetInfoUrl)，栏目URL(GetClassUrl) 5种类型<br><font color=red>3、支持使用{ReqNum(字符串)}或{ReqStr(字符串)}来取得Url的参数值</font><br>如：http://www.kesion.com/index.asp?ClassID=100,那么{ReqNum(ClassID)} 将得到100<br/><font color=red>4.当个人/企业空间要使用sql标签时,可以用<font color=red>""{$GetUserName}""</font>取得当前空间的用户名 </font><br>如:select top 10 id,title from ks_article where inputer='{$GetUserName}' order by id desc"
...
Sub AddLabelSave()
			...
			Else
				...
				LabelRS.AddNew
				 ...
				 KS.echo ("<script>if (confirm('成功提示:\n\n添加标签成功,继续添加标签吗?')){location.href='LabelFunctionAdd.asp?Action=AddNew&LabelType=5&FolderID=" & FolderID & "';}else{$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 自义定函数标签&ButtonSymbol=DIYFunctionLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=5&FolderID=" & FolderID & "';}</script>")
			End If
	End Sub
	...
	Sub EditLabelSave()
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
					KS.Echo ("<script>alert('成功提示:\n\n标签修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理  >> 自定义函数标签&ButtonSymbol=DIYFunctionLabel';location.href='Label_main.asp?Page=" & Page & "&LabelType=5&FolderID=" & FolderID & "';</script>")
				 Else
					KS.Echo ("<script>alert('成功提示:\n\n标签修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?OpStr=标签管理 >> <font color=red>搜索自定义函数标签结果</font>&ButtonSymbol=DIYFunctionSearch';location.href='Label_main.asp?Page=" & Page & "&LabelType=5&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';</script>")
				 End If
			End If
	End Sub
