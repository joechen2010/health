...
Class LabelAdd
		...
		Select Case Request.Form("Action")
		 Case "AddNewSubmit"
			...
			Else
				...
				.Write ("<script>if (confirm('成功提示:\n\n添加标签成功,继续添加标签吗?')){location.href='LabelAdd.asp?Action=AddNew&mode=text&LabelType=1&FolderID=" & FolderID & "';}else{$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 自定义静态标签&ButtonSymbol=FreeLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=1&FolderID=" & FolderID & "';}</script>")
			End If
		Case "EditSubmit"
			...
			Else
				...
				LabelRS.Open "Select * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
					.Write ("<script>alert('成功提示:\n\n标签修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & ParentID & "&OpStr=标签管理  >> 自定义静态标签&ButtonSymbol=FreeLabel';location.href='Label_main.asp?Page=" & Page & "&LabelType=1&FolderID=" & ParentID & "';</script>")
				 Else
					.Write ("<script>alert('成功提示:\n\n标签修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?OpStr=标签管理 >> <font color=red>搜索自定义静态标签结果</font>&ButtonSymbol=FreeLabelSearch';location.href='Label_main.asp?Page=" & Page & "&LabelType=1&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';</script>")
				 End If
			End If
		End Select
		...
		.Write "      <td>" & KS.ReturnLabelFolderTree(FolderID, 1) & "<font color=""#FF0000"">请选择标签归属目录，以便日后管理标签</font></td>"
		.Write "    </tr>"
		.Write "    <tr class=""tableBorder1"">"
		...
		If KS.G("Mode")="text" then
		 ...
		 Response.Write "&nbsp;<input type=""button"" onclick=""javascript:LabelInsertCode();"" value=""选择更多标签"">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</Td>"
		 Response.Write "      </Tr>"
		...
		If KS.G("Mode")<>"text" then
		.Write "  if (frames[""LabelEditor""].CurrMode!='EDIT') {alert('其他模式下无法保存，请切换到设计模式');return false;}"
