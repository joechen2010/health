...
<tr>
	...
	<%
	 Call KSCLS.ShowPage(totalPut, MaxPerPage, "KS.MallScoreOrder.asp", True, "个", CurrentPage, KS.QueryParam("page"))
	%></td>
</tr>
</table>
<div>
	   ...
	   If RealName="" Then Response.Write "<script>alert('收货人必须输入');history.back();</script>":response.end
            ...
            Dim RS: Set RS=Server.CreateObject("Adodb.RecordSet")
			  RS.Open "Select top 1 * From KS_MallScoreOrder Where ID=" & ID,Conn,1,3
				...
				if Status=5 then
				   '更新用户积分
				   Call KS.ScoreInOrOut(RS("UserName"),1,KS.ChkClng(KS.G("Score"))*RS("Amount"),"系统","返回兑换订单号<font color=red>" & RS("OrderID") & "</font>的礼品积分!",0,0)
				end if
