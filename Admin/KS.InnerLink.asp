...
Class Inner_Main
	...
	%>
	   <script language="javascript">
		...
		function InnerAdd()
		{
			PopupCenterIframe('添加新站内链接','KS.InnerLink.asp?Action=Add',630,250,'no')
		}
		function EditInner(id)
		{
			PopupCenterIframe('添加新站内链接',"KS.InnerLink.asp?Action=Edit&Flag=Edit&InnerID="+id,630,250,'no')
		}
		  ...
		  SqlStr = "SELECT * FROM KS_InnerLink order by AddDate desc"
		   ...
		   CloseConn
	        ...
	        .echo ("</form><td align='right'>")
	         Call KSCls.ShowPage(totalPut, MaxPerPage, "KS.InnerLink.asp", True, "位", CurrentPage, "")
	        .echo ("</td></tr></table>")
			.echo "</body>"
		 .echo "</html>"
		 End With
		...
		Sub showContent()
		 ...
		 If IsObject(XML) Then
		   For Each Node In XML.DocumentElement.SelectNodes("row")
				...
				.echo "  <td class='splittd' height='20'> &nbsp;&nbsp; <span InnerID='" & ID & "' ondblclick=""EditInner(this.InnerID)""><img src='Images/Inner.gif' align='absmiddle'>"
		...
		Sub InnerAdd()
			   With KS
					...
					End If
					 ...
					 .echo "        <td><input name=""Title"" type=""text"" id=""Title"" value=""" & Title & """ class=""textbox"" style=""width:50%""> 如AspCMS,KesionCMS等</td>"
					 .echo "    </tr>"
					 .echo "    <tr class=""tdbg"">"
					...
					Else
					   .echo "http://"
					End If
					 .echo """ class=""textbox"" style=""width:50%""> 如:http://www.kesion.com</td>"
					 .echo "    </tr>"
					 .echo "    <tr class=""tdbg"">"
			 ...
			 Sub InnerAddSave()
				...
				If Flag = "Add" Then
				  ...
				  End If
				   Set RSObj = Nothing
				    KS.Echo ("<script> if (confirm('站内链接添加成功!继续添加吗?')) {location.href='KS.InnerLink.asp?Action=Add';}else{top.frames[""MainFrame""].location.reload();}</script>")
				ElseIf Flag = "Edit" Then
				  ...
				  Else
				   ...
				   Set RSObj = Nothing
				  End If
				  Response.Write ("<script>alert('站内链接修改成功!');top.frames[""MainFrame""].location.reload();</script>")
				End If
			 End Sub
