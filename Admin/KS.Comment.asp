...
Class Admin_Comment
		...
		Sub DelAllRecord()
          ...
          KS.echo "<script>$(top.frames['FrameTop'].document).find('#ajaxmsg').toggle();alert('恭喜,删除指定日期评论成功!');</script>"
		 End Sub
...
<%
	...
	'  .echo "<body scroll=no topmargin=""0"" leftmargin=""0"" onkeydown=""GetKeyDown();"" onselectstart=""return false;"">"
	  ...
	  .echo "<li></li><form action='?' method='post'><div>条件:<select name='searchtype'><option value='1'>文档标题</option><option value='2'>评论者</option><option value='3'>评论内容</option></select>关键字:<input type='text' class='textbox' name='keyword'> <input class='button' type='submit' value=' 搜 索 '></div></form></ul>"
	  ...
	  .echo ("</td>")
	  .echo ("<td><select style='height:18px' onchange='set(this.value)' name='setattribute'><option value=0>快速设置...</option><option value='1'>设为已审</option><option value='2'>设为未审</option><option value='3'>执行删除</option></select></td>")
	  ...
	  If IsObject(CommentXml) Then
		  For Each Node In CommentXml.DocumentElement.SelectNodes("row")
			  ID=Node.SelectSingleNode("@id").text
			    ...
			    .echo "  <td height='20' class='splittd' title='双击查看详细内容'><span CommentID='" & ID & "' ondblclick=""this.submit()"" title=""" & Node.SelectSingleNode("@content").text & """><img src='Images/t4.gif' align='absmiddle'>"
			  ...
			  End If
			    .echo "  <td align='center' class='splittd'><a href='KS.Comment.asp?Action=View&ChannelID=" & ChannelID & "&CommentID=" & ID & "'>查看/回复</a>  <a href='KS.Comment.asp?ChannelID=" & ChannelID & "&Action=Del&CommentID=" & ID & "' onclick=""return(confirm('确定删除吗?'))"">删除</a></td>"
			    .echo "</tr>"	  
		  Next
	  End If
	 End With
    ...
    Sub CommentDel()
		 ...
		 Sub ProcessUserScore(RS)
		      If Cint(RS(1))=0 And DateDiff("m",RS(4),Now)<1 Then
				 ...
				 If Not RSU.Eof Then
			        ...
			        Conn.Execute("Insert into KS_LogScore(UserName,InOrOutFlag,Score,CurrScore,[User],Descript,Adddate,IP,Channelid,InfoID) values('" & RS(0) & "',2," & KS.ChkClng(KS.U_S(RSU("GroupID"),6)) & ","&CurrScore & ",'系统','评论[" & KS.GotTopic(KS.HTMLEncode(RS(3)),36) & "]被删除!'," & SqlNowString & ",'" & replace(ks.getip,"'","""") & "',1002," & RS("ChannelID") & RS("InfoID") & ")")
        ...
        With KS
			...
			End If
				  ...
				  .echo "          <tr class='tdbg'>"
				  .echo "            <td width=""200"" class='clefttitle' height=""25""> <div align=""center"">" & KS.C_S(RSObj("ChannelID"),3) &"标题</div></td>"
				  .echo "            <td> "
				  ...
				  .echo "          </td></tr>"
				  .echo "          <tr class='tdbg'>"
				  .echo "            <td class='clefttitle' height=""25""> <div align=""center"">发表人</div></td>"
				  .echo "            <td> " & RSObj("UserName") & "</td>"
				  .echo "          </tr>"
				'  .echo "          <tr>"
				'  .echo "            <td width=""200"" height=""25""> <div align=""center"">电子邮箱</div></td>"
				'  .echo "            <td width=""542""> <a href=""MailTo:" & RSObj("Email") & """ target=""_blank"">" & RSObj("Email") & "</a></td>"
				  .echo "          </tr>"
				  .echo "          <tr class='tdbg'>"
				  .echo "            <td height=""25"" align=""center"" class='clefttitle'>发表IP</td>"
				  .echo "            <td>" & RSObj("UserIP") & "</td>"
				  .echo "          </tr>"
				  .echo "          <tr class='tdbg'>"
				  .echo "            <td height=""25"" class='clefttitle' align=""center"">发表时间</td>"
				...
				End If
				  ...
				  .echo "        <input type=""button"" name=""Submit1"" onclick=""javascript:window.open('" & Url & "','new','');"" value=""查看" & KS.C_S(ChannelID,3) &""" class='button'>"
				  .echo "      </td>"
				  .echo "    </tr>"
				  .echo "</form>"
				  .echo "  </table>"
